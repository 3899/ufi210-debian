...
if ($Manifest.target_partition -ne "system" -or $Manifest.usb_product_id -ne "0xD001" -or
...
Write-Utf8File (Join-Path $OutputDir "before.txt") ((@(
    "boot_id=$beforeBootId", "adapter_guid=$($beforeAdapter.InterfaceGuid)",
    "adapter_mac=$($beforeAdapter.MacAddress)", "usb_instances_begin", $beforeFingerprint,
    ...
    try {
        if ((Get-DebianUsbDevices).Count -eq 3 -and (Test-TcpAdb)) {
            ...
            if ($afterAdapter.Status -ne "Up" -or
                $afterAdapter.InterfaceGuid -ne $beforeAdapter.InterfaceGuid -or
                $afterAdapter.MacAddress -ne $beforeAdapter.MacAddress -or
                -not (Test-TcpPort 22)) {
